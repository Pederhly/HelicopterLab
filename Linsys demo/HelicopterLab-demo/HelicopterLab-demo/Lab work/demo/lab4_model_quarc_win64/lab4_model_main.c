...
typedef RT_MODEL_lab4_model_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * lab4_model(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(lab4_model_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(lab4_model_DW.StreamCall2_Stream);
  }

  hil_task_stop(lab4_model_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://lab4_model:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: lab4_model -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(lab4_model_DW.HILReadEncoderTimebase_Task,
          (t_clock) lab4_model_P.HILReadEncoderTimebase_Clock, 500.0, -1)
          ;
        if (result == 0) {
