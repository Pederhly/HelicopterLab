...
typedef RT_MODEL_lab3_model_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * lab3_model(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(lab3_model_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(lab3_model_DW.StreamCall2_Stream);
  }

  hil_task_stop(lab3_model_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://lab3_model:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: lab3_model -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(lab3_model_DW.HILReadEncoderTimebase_Task,
          (t_clock) lab3_model_P.HILReadEncoderTimebase_Clock, 500.0, -1)
          ;
        if (result == 0) {
